...
class GettingData{
    ...
    func createListMovie()->Decodable{
    ...
    let requestData =  AF.request("https://api.themoviedb.org/3/discover/movie?api_key=6cde63f94256f35e302a61f1dd4b7524&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=true&page=1&with_watch_monetization_types=flatrate", method: .get).responseDecodable(of: MovieModel.self) { response in
        
        do {
            let result = try response.result.get().self
